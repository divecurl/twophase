subroutine dilute_flux4_u2ph(fluxL,fluxR,&
    dL,eL,pL,cL,vxL,vyL,vzL,vofL,ppL_gas_in,uuL_solid_in,del_uL_solid,keL,&
...
!  ppR_gas= ppR_gas_in
  ...
  vof_cc(2)= vofL(1)-vofR(1)

  if(vof_cc(2).lt.-1.d-11) then
     print *, "+ inreasonable vof_cc(2) : =", vof_cc(2)
     pause
     stop
  endif
...
!     end if
     ...
     p_sum= -pp_gas_itf*vof_cc(2)-ppL_gas_vis*vofL(2)+ppR_gas_vis*vofR(2)

     ! flux on left
     fluxL(7,1) = (u_itf-uuL_solid)*vofL(2)
     fluxL(1,1) = u_itf*vof_cc(2)+fluxL(7,1)

     fluxL(3,1)= -pp_gas_itf*vof_cc(2)
     fluxL(6,1)= -pp_gas_itf*u_itf*vof_cc(2)-ppL_gas*fluxL(7,1)
...
!     if(uuL_solid.gt.uuR_solid) then
        ...
        !tmp1 = vofL(2)*vofR(2)/(vofL(2)+vofR(2))
        tmp1 = 1.d0
...
end subroutine dilute_flux4_u2ph
